...
def actualizar_informe(
    ...
    # Solo el creador original o la directora pueden cambiar Comisión 1
    if 'comision_usuario1_id' in cambios:
        if current_user.rol != 'directora' and str(inf.registrado_por) != str(current_user.id):
            # Ignorar el cambio de comision_usuario1_id, mantener el original
            cambios.pop('comision_usuario1_id', None)
